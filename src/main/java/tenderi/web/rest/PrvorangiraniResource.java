...
import tenderi.service.PrvorangiraniQueryService;
import tenderi.service.PrvorangiraniService;
import tenderi.service.criteria.PrvorangiraniCriteria;
import tenderi.web.rest.errors.BadRequestAlertException;

/**
 * REST controller for managing {@link tenderi.domain.Prvorangirani}.
 */
@RestController
@RequestMapping("/api")
public class PrvorangiraniResource {
    ...
    public ResponseEntity<List<Prvorangirani>> getAllPrvorangiranis(PrvorangiraniCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Prvorangiranis by criteria: {}", criteria);
        Page<Prvorangirani> page = prvorangiraniQueryService.findByCriteria(criteria, pageable);
